...
class TestObsHourIndex:
    ...
    def test_midnight(self) -> None:
        """Confirm that a midnight valid time (00:00 UTC) maps to observation hour index 23, referencing the last band of the preceding day's FIMERG file. Midnight accumulations complete the 24-hour period that began at 01:00 UTC the previous day, so they are stored in the prior day's file rather than the current day's. This test prevents the common error of associating midnight data with index 0 of the current day."""
...
class TestGroupObsTimes:
    ...
    def test_single_day(self) -> None:
        """Verify that a sequence of valid times falling within a single calendar day are all grouped under the correct date key. This test generates six hourly times from 01:00 to 06:00 on 2025-06-14 and asserts they are placed in a single group keyed by '20250614'. Accurate single-day grouping is the baseline behavior required before testing more complex midnight boundary cases."""
