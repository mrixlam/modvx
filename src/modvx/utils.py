...
def parse_datetime_string(datetime_str: str) -> datetime.datetime:
    ...
    Parameters:
        datetime_str (str): Human-readable datetime string in ``yyyymmddThh`` or ISO-8601 format
...
def parse_fss_filename_metadata(filename: str) -> dict | None:
    """
    Extract domain, threshold, and window metadata from a standardised FSS NetCDF filename. The expected filename pattern is ``<domain>_FSS_<Nh>_indep_thresh<T>percent_window<W>.nc``, where threshold values may use ``p`` in place of ``.`` (e.g. ``97p5`` for 97.5). This function is used during CSV extraction to parse result files without opening them. Returns ``None`` when the filename does not conform to the expected pattern.
    ...
    domain_match = re.match(r"^([^_]+(?:_[^_]+)*?)_FSS_", name)
    if not domain_match:
        return None
...
# Legacy short-name aliases removed — use descriptive helpers:
# `parse_datetime_string`, `iterate_valid_times`,
# `iterate_forecast_cycle_starts`, `parse_fss_filename_metadata`, and
